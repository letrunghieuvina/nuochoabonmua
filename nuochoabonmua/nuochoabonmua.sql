...
CREATE TABLE PhanHoiKhachHang(MaPhanHoiKhachHang INTEGER PRIMARY KEY, MaKhachHang INT, NgayPhanHoi DATETIME, NoiDung TEXT, LinkFaceBook TEXT, TotXau TINYINT); /*TotXau: 1 = tot; 2 = xau; 3 = tot + xau*/
/*LinkFaceBook: link toi phan hoi cua k/h tren facebook*/

CREATE TABLE SanPham(MaSanPham INTEGER PRIMARY KEY, TenSanPham VARCHAR(100), MaLoaiSanPham INT, NamNu TINYINT, DungTich SMALLINT, MaHangSanXuat INT, GhiChu VARCHAR(255)); /* Name = 1; Nu = 0 */
...
CREATE TABLE ChiTietDonNhapHang(MaChiTietDonNhapHang INTEGER PRIMARY KEY, MaDonNhapHang INT, MaSanPham INT, GiaNhap INT, SoLuong INT); /* KhuyenMai: 0 = MUA, 1 = KHUYEN MAI; TrangThai: 0 = DA BAN, 1 = CON HANG */
...
CREATE TABLE ChiTietDonHang(MaChiTietDonHang INTEGER PRIMARY KEY, MaDonHang INT, MaSanPham INT, MaGiaBan INT, QuaTang TINYINT);
